'''
Pure PyTorch implementation of 2D oriented-box intersection
...
from .vertex_sort import sort_vertices

EPSILON = 1e-8
BOX_IN_BOX_EPSILON = 1e-6
...
def box1_in_box2(corners1:torch.Tensor, corners2:torch.Tensor):
    ...
    tol = BOX_IN_BOX_EPSILON
    cond1 = (p_ab / norm_ab > -tol) * (p_ab / norm_ab < 1 + tol)   # (B, N, 4)
    cond2 = (p_ad / norm_ad > -tol) * (p_ad / norm_ad < 1 + tol)   # (B, N, 4)
    return cond1*cond2
...
def sort_indices(vertices:torch.Tensor, mask:torch.Tensor):
    ...
    return sort_vertices(vertices_normalized, mask, num_valid).long()
