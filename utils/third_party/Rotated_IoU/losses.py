...
from .intersection import oriented_box_intersection_2d
from .enclosing_box import smallest_bounding_box


def boxes_to_corners(box: torch.Tensor) -> torch.Tensor:
...
box2corners_th = boxes_to_corners


def _cal_iou(box1:torch.Tensor, box2:torch.Tensor):
    """calculate iou
    ...
    """
    corners1 = boxes_to_corners(box1)
    corners2 = boxes_to_corners(box2)
...
def _cal_my_iou(box1:torch.Tensor, box2:torch.Tensor):
    """calculate iou
    ...
    Returns:
        iou (torch.Tensor): (B, N)
        area1 (torch.Tensor): (B, N)
        area2 (torch.Tensor): (B, N)
        inter_area (torch.Tensor): (B, N)
        corners1 (torch.Tensor): (B, N, 4, 2)
    ...
    u = area1 + area2 - inter_area
    iou = inter_area / u
    return iou, area1, area2, inter_area, corners1, corners2
    # return 1. - iou, iou

def cal_iou(box1:torch.Tensor, box2:torch.Tensor):
    """calculate iou loss
    Minimizing `iou_loss` maximizes overlaps
    ...
    Returns:
        iou_loss (torch.Tensor): (B, N). 
        iou (torch.Tensor): (B, N), raw iou value
    """
    iou, _, _, _ = _cal_iou(box1, box2)
    iou_loss = 1. - iou
    return iou_loss, iou
...
def cal_diou(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    ...
    Args:
        ...
        box2: (B, N, 5),
        last dimension: 
            - [x_center, y_center, width, height, angle (or alpha)]
    """
    iou, corners1, corners2, u = _cal_iou(box1, box2)
...
def cal_my_iou(box1:torch.Tensor, box2:torch.Tensor):
    """calculate iou loss
    Minimizing `iou_loss` maximizes overlaps
...
def cal_my_diou(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    """calculate diou loss
    ...
    Args:
        ...
        last dimension: 
            - [x_center, y_center, width, height, angle (or alpha)]
    """
    iou, area1, area2, inter, corners1, corners2 = _cal_my_iou(box1, box2)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    c2 = w*w + h*h      # (B, N)
    x_offset = box1[...,0] - box2[..., 0]
    y_offset = box1[...,1] - box2[..., 1]
    d2 = x_offset*x_offset + y_offset*y_offset
    min_area = torch.minimum(area1, area2) # (B, N)
    max_overlap = inter / min_area # (B, N)
    diou_loss = iou - (d2/c2) * max_overlap.detach()
    return diou_loss, iou


def cal_my_giou(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    iou, corners1, corners2, u = _cal_iou(box1, box2)
    w, h = enclosing_box(corners1, corners2, enclosing_type)
    area_c =  w*h
    giou_loss = iou - ( area_c - u )/area_c
...
def cal_iou_3d(box3d1:torch.Tensor, box3d2:torch.Tensor, verbose=False):
    ...
    iou_2d, corners1, corners2, u = _cal_iou(box1, box2)        # (B, N)
