...
from utils.holodeck_v2.constants import (
    OBJATHOR_ANNOTATIONS_PATH,
    OBJATHOR_FEATURES_DIR,
)
from utils.holodeck_v2.utils import get_bbox_dims_vec
from utils.models import CLIP, clip_weights


class SlimRetriever:
    """
    Lightweight retriever that only loads the annotation database.
    Use this when assets are pre-specified and retrieval is not needed.
    """
    ...
    def __init__(self):
        try:
            ...
            print(f"Error loading objathor annotations: {e}")
            objathor_annotations = {}

        self.database = objathor_annotations


class HolodeckRetrieverV2:
    def __init__(
        self,
        ...
        try:
            ...
            print(f"Error loading objathor annotations: {e}")
            print(f"Hint: might need to check OBJATHOR_ASSETS_BASE_DIR in constants.py. Currently set to {OBJATHOR_ANNOTATIONS_PATH}")
            objathor_annotations = {}
    ...
    def _load_clip_features_and_check_has_text(self, clip_features_path: str):
        with FileLock(clip_features_path + ".lock"):
            ...
            clip_model, _, _ = open_clip.create_model_and_transforms(
                model_name=CLIP.arch, pretrained=clip_weights(CLIP), device=DEFAULT_DEVICE
            )
    ...
    def retrieve(
        ...
        with torch.no_grad():
            ...
            img_query_features_clip = F.normalize(img_query_features_clip, p=2, dim=-1)  # [num_queries, 768]

            text_query_features_clip = self.clip_model.encode_text(
                self.clip_tokenizer(text_queries)
            )
            text_query_features_clip = F.normalize(  # [num_queries, 768]
                text_query_features_clip, p=2, dim=-1
            )

        clip_img_similarities = 100 * torch.einsum(  # [num_queries, num_assets, 3]
            "ij, lkj -> ilk", img_query_features_clip, self.clip_img_features
        )
        clip_img_similarities = torch.max(clip_img_similarities, dim=-1).values  # [num_queries, num_assets]

        clip_text_similarities = 100 * torch.einsum(  # [num_queries, num_assets]
        ...
        results: List[Tuple[str, float]] = sorted(unsorted_results, key=lambda x: x[1], reverse=True)
